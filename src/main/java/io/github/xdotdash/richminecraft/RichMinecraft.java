...
@Mod(modid = RichMinecraft.MOD_ID, name = RichMinecraft.MOD_NAME, version = RichMinecraft.VERSION, clientSideOnly = true)
public class RichMinecraft {

    public static final String MOD_ID = "richminecraft";
    public static final String MOD_NAME = "RichMinecraft";
    public static final String VERSION = "1.0-SNAPSHOT";

    @Mod.Instance(MOD_ID)
    public static RichMinecraft INSTANCE;
    ...
    public void onPreInit(FMLPreInitializationEvent event) {
        Path configPath = event.getModConfigurationDirectory().toPath().resolve("RichMinecraft.json");

        if (Files.notExists(configPath)) {
            try {
                Files.copy(getClass().getResourceAsStream("RichMinecraft.json"), configPath);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
